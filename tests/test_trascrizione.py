...
def finto_claude(tmp_path, monkeypatch):
    ...
    script.write_text(
        "#!/bin/sh\n"
        f"echo chiamata >> {registro_chiamate}\n"
        f"N=$(wc -l < {registro_chiamate})\n"
        f'{sys.executable} -c "'
        f"import json,sys; c=json.load(open(r'{copione}')); "
        f'i=min(int(sys.argv[1])-1, len(c)-1); print(json.dumps(c[i]))" $N\n',
        encoding="utf-8",
    )
